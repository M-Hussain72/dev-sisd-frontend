...
import { CartItemsType, useCart } from '../context/CartContext';

export default function CartCard({ product }: { product: CartItemsType }) {
  const { removeToCart } = useCart();
  return (
    <div className="flex justify-between   gap-4 items-start   ">
      <div className=" max-w-[130px] mt-1">
        <img src={image2} alt="courseImage" className=" min-w-[50px]" />
      </div>
      <div className="block w-full ">
        <div className=" flex justify-between  w-full min-w-[200px]  gap-4 ">
          <div className=" max-w-[550px] ">
            <div className="">
              <p className=" text-[#2B2B2B] text-lg font-medium line-clamp-3 ">{product.name}</p>
            </div>
            <div>
              <div className="flex mt-1">
                ...
                />
                {/* no of reviews */}
                <span className=" ml-1 h-fit text-xs text-[#949697] cursor-default">(3203400)</span>
              </div>
            </div>
            <div className=" group flex items-center gap-1 my-[2px]  ">
              <svg
                width="20"
                height="20"
                viewBox="0 0 20 20"
              ...
              <p className="  cursor-default text-sm text-[#949697] ">Watson Holmes</p>
            </div>
          </div>
          <div className=" ">
            <div>
              <p className=" text-themeBlack text-lg font-semibold ">
                ${product.discountPrice > 0 ? product.discountPrice : product.originalPrice}
              </p>
              {product.discountPrice > 0 && (
                <p className=" text-themeGray line-through text-sm font-semibold ">${product.originalPrice}</p>
              )}
            </div>
          </div>
        </div>
        <div className=" flex items-end justify-between">
          <div className=" flex flex-wrap items-center text-themeGray">
            <p className="xs:text-sm text-xs ">32 total hours</p>
            <span className="text-xl">•</span>
            <p className="xs:text-sm text-xs">23 lectures</p>
            <span className="text-xl">•</span>
            <p className="xs:text-sm text-xs">All Levels</p>
          </div>
          <button className=" text-indigo-800 font-medium" onClick={() => removeToCart(product)}>
            Remove
          </button>
        </div>
      </div>
