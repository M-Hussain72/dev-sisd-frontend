...
import InputFelid from './ui/InputFelid.tsx';
...
export default function SignUp() {
  ...
  function onSubmit() {
    setTimeout(() => {
      const { referralName, ...fromData } = values;
      mutate({ fromData });
      setSubmitting(false);
    }, 400);
  }
  ...
  return (
    <>
      <div className=" xs:w-[440px] w-[300px] my-10 mx-auto">
        <form onSubmit={handleSubmit} noValidate>
          <h1 className=" font-semibold text-xl text-themeBlack">Sign Up and Start Learning</h1>
          {isError && <ErrorBlock title="Registration Unsuccessful!" message={error.message} />}
          ...
          <Button disabled={isSubmitting || isPending} type="submit">
            <text className=" xs:px-[170px] px-[114px]">Sign Up</text>
          </Button>
        </form>
        <p className="xs:text-sm text-xs font-light text-themeGray cursor-default mt-4">
